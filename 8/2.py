...
class Exercise8:
    ...
    def counter(self, len_matrix: int, line: int = 0) -> None:
        if line != 0 and line != len(self.matrix):
            for item in range(1, len_matrix):
                left_distance = self.find_distance(self.matrix_copy[line][0:item][::-1], self.matrix_copy[line][item])
                rigth_distance = self.find_distance(self.matrix_copy[line][item+1:len_matrix + 1], self.matrix_copy[line][item])
                if self.matrix[line][item] != 0:
                    self.matrix[line][item] *= left_distance * rigth_distance
                    if self.count < self.matrix[line][item]:
                        self.count = self.matrix[line][item]
                else:
                    self.matrix[line][item] = left_distance * rigth_distance
    ...
    def find_distance(self, item_list: list, number: int):
        distance = 0
        for x in item_list:
            distance += 1
            if x >= number:
