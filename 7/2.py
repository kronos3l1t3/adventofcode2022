...
class Tree:
    ...
    def remainin_dir(self, rest, children=None):
        ...
        for node in children:
            if len(node.children) > 0:
                min_node = node.remainin_dir(rest, node.children)
                if min_node is not None and min_node < menor:
                    menor = min_node
            elif node.data - rest > 0 and node.data < menor:
                menor = node.data
        return menor
...
class Exercise7:
    ...
    def seek_file_system(self):
        try:
            ...
            tree.recalc_totals()
            print(tree.remainin_dir(30000000 - (70000000 - int(tree.data))))
